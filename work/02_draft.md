...
エンジニアがアウトプットをしない理由を聞くと、大抵この3つに集約されます。
...
「間違ったことを書いたら恥ずかしい」「批判されたら……」。この感情は自然ですが、多くの場合、見に来るのはあなたと同じ悩みを持つエンジニアたちです。
...
このモデルにおける学びのポイントは**上流工程**にあります。

- 「誰に」「何を」「どんな目的で」伝えるか——これを考える過程で、技術への理解が深まります
...
- **ハルシネーション**：存在しないAPIや廃止されたライブラリの情報が混入する
...
それぞれが**特定の観点のみ**に集中することで、品質が格段に上がります。
...
マルチエージェントパイプラインは複数回のAPI呼び出しが発生します。1記事あたりの目安コストは使用モデルによって異なります。`claude-haiku-4-5`を使えばコストを抑えられますが、品質は`claude-sonnet-4-6`の方が高くなります。最初は`claude-haiku-4-5`でテストすることをおすすめします。
:::
...
def run_pipeline(input_path: str = "input.json"):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        ...
        concurrent.futures.wait([future_critic, future_reader])
...
- 技術的な事実確認（Stage 3a）で、ハルシネーションが1〜3件程度検出された
- コードレビュー（Stage 3b）で、セキュリティ上の注意点が追加された
