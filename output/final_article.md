...
:::note info
**Stage 8（図示化）の自動化スコープについて**

パイプラインが自動で行うのは「画像生成プロンプトの出力まで」です。実際の画像生成はGemini（NanoBanana2）への入力が必要なため、**ここだけ手動ステップが1つ入ります**。プロンプトをコピペして貼り付けるだけなので、作業自体は数分で完了します。

「完全無人自動化」と誤解しないよう注意してください。
:::
...
9ステージを全て自動で走らせることは可能ですが、「中間ファイルを全部読まないといけないのか？」という疑問は自然です。答えはNoです。**全ての中間ファイルに目を通す必要はありません。**

筆者が推奨する運用は、**Stage 6（修正Todo）の出力だけ人間がレビューしてGoサインを出す**ゲート設計です。

| ステージ | 人間の介入度 | 理由 |
|---------|-----------|-----|
| Stage 1〜5 | 基本的にAI任せ | 中間ファイルは参考程度で十分 |
| **Stage 6（Todo洗い出し）** | **必ず確認する** | 修正方針の取捨選択は人間の判断が必要 |
| Stage 7〜9 | 基本的にAI任せ | Goサイン後は自動で完走 |
| 最終成果物 | 必ず確認・加筆する | 個人体験・ファクトチェックを人間が補完 |

`work/06_todos.md` の修正リストだけ確認し、方向性に納得したら Stage 7 へ進む——これが「単一プロンプトへの丸投げ」と本パイプラインを区別するポイントです。
...
コストを抑えるコツとして、Stage 1〜6の中間ステージは`claude-haiku-4-5-20251001`、最終的な執筆（Stage 2）と修正（Stage 7）だけ`claude-sonnet-4-6`を使う**ハイブリッド構成**が有効とのことです。
